/*
	...
	Copyright (C) 2002-2025 Svante Seleborg/Axantum Software AB. All rights reserved.
...
----
	XecretsFileClassic.cpp          WinMain() and friends.
...
/*
	...
	The extension .xxx is registered to start Xecrets File Classic which
	...
	with the action "Encrypt and Wrap with Xecrets File Classic", as well as "Decrypt and Unwrap from Xecrets File Classic".
		...
		if (tArgc > 1) {
			...
			if (ParseImmediateCommand(nCmdShow, &dwExitCode)) {
				...
				if (ghMutex == NULL) {
					...
					// that for version 2.0. Not to be forgotten is that the caller of Xecrets File Classic
