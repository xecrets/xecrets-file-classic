...
public:
    CPassphraseBase(axcl::CAxCryptLib *pAxCryptLib = NULL) : base(pAxCryptLib), m_pdlgKeyFile(NULL)
    {
...
public:
    CEncryptPassphrase(axcl::CAxCryptLib *pAxCryptLib = NULL) : base(pAxCryptLib) {
    }
        ...
        if (nID == IDC_EDIT_PASSPHRASE_2) {
            ...
            if (bSuccess && bSave) {
                if (_tcscmp(m_Passphrase1, m_Passphrase2) != 0) {
                    ...
                    ASSCHK(m_pAxCryptLib->HashKey(AXCL_KEY_ENC, reinterpret_cast<const unsigned char *>(sPassphrase.get()), strlen(sPassphrase.get()), GetKeyFileName()) == AXCL_E_OK, m_pAxCryptLib->GetError().c_str());
                }
		    }
        }
        return bSuccess;
...
public:
    CDecryptPassphrase(axcl::CAxCryptLib *pAxCryptLib = NULL) : base(pAxCryptLib) {
    }
...
protected:
	...
	BOOL DDX_Text(UINT nID, CString& strText, int cbSize, BOOL bSave, BOOL bValidate = FALSE, int nLength = 0) {
        ...
        if (bSuccess && bSave) {
            ...
            ASSCHK(m_pAxCryptLib->HashKey(AXCL_KEY_DEC, reinterpret_cast<const unsigned char *>(sPassphrase.get()), strlen(sPassphrase.get()), GetKeyFileName()) == AXCL_E_OK, m_pAxCryptLib->GetError().c_str());
            // Check the key and get the decrypted meta-data
            int iError = m_pAxCryptLib->DecryptFileMeta(AXCL_KEY_DEC, m_pAxCryptLib->GetThisCipherPath().c_str());
            if (iError != AXCL_E_OK) {
                // Ensure that it really was the key, and not something else.
                ASSCHK(iError == AXCL_E_WRONGKEY, m_pAxCryptLib->GetError().c_str());
