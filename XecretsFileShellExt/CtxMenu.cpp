...
#define VERB_CZECH      "czech"             ///< Select user interface language to czech
#define VERB_FINNISH    "finnish"           ///< Select user interface language to finnish
...
struct CShellExt::SVerbs CShellExt::m_Verbs[] = {
	...
	{VERB_FINNISH, INF_MENU_FINNISH, HLP_MENU_LANGUAGE, 0, &CShellExt::DoFinnish},
	...
	try {
		...
		if (((uFlags & CMF_EXPLORE) || ((uFlags & 0xf) == CMF_NORMAL)) && !(uFlags & CMF_DEFAULTONLY)) {
			try {
				...
				SetVerb(m_Verbs, VERB_FINNISH, idCmd - idCmdFirst);
				InsertMenu(hLanguageSubMenu, -1, MF_STRING | MF_BYPOSITION, idCmd, CMessage().AppMsg(GetMenuMsgId(m_Verbs, idCmd - idCmdFirst)).GetMsg());
				idCmd++;
...
DWORD
CShellExt::DoFinnish(itEventT eventId, HWND hProgressWnd, IShellFolder* pShellFolder, LPCITEMIDLIST pidlFile, CParam** ppParam) {
	return DoLanguage(eventId, 1035);
}
